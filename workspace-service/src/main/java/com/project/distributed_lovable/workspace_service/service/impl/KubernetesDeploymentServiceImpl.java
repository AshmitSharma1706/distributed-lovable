...
public class KubernetesDeploymentServiceImpl implements DeploymentService {
    ...
    private DeployResponse claimAndStartNewPod(Long projectId, String domain, String formattedUrl) {
        ...
        try {
            String initialSyncCmd = String.format("rm -rf /app/* && mc mirror --overwrite myminio/lovable-ai/%d/ /app/", projectId);
            execCommand(podName, "syncer", "sh", "-c", initialSyncCmd);

            String watchCmd = String.format("nohup mc mirror --overwrite --watch myminio/lovable-ai/%d/ /app/ > /app/sync.log 2>&1 &", projectId);
...
//                    "mc mirror --overwrite myminio/lovable-ai/%d/ /app/",
//                    projectId);
//
...
//                    "nohup mc mirror --overwrite --watch myminio/lovable-ai/%d/ /app/ > /app/sync.log 2>&1 &",
//                    projectId);
